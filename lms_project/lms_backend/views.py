...
def _convert_word_template_to_pdf_bytes(template_file):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Try docx2pdf first when available (works well on Windows with MS Word).
        if ext == '.docx':
            try:
...
def send_html_email(subject, html_content, recipient_list, from_email=None):
    """
    Helper function to send HTML emails with proper error handling
    """
    try:
...
def generate_welcome_email_staff(first_name, email, password, site_url):
    ...
    <head>
        ...
        <style>
            ...
            .otp-code {{ font-size: 28px; font-weight: bold; color: #667eea; text-align: center; padding: 20px; background-color: #f0f0f0; border-radius: 8px; letter-spacing: 5px; }}
...
def generate_certificate_email(student_name, course_title, certificate_id, custom_message=None):
    """Generate HTML email content for certificate sharing"""
...
def student_credit_points(request):
    ...
    STREAK_BONUS_SHORT = 10   # 3+ day streak
    STREAK_BONUS_LONG = 25    # 7+ day streak
    ...
    # Engagement and quiz-based points
    ...
    # 2 points for each attempt to encourage participation
    quiz_attendance_points = quiz_attempts_count * 2
    quiz_score_total = sum(int(a.score or 0) for a in quiz_attempts_qs)
    # Score normalization: each 20 score points contributes 1 credit
    quiz_score_points = int(quiz_score_total // 20)

    # Daily activity points only count actual learning activity days.
    first_credit_activity_at = _get_first_credit_activity_at(user)
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if request.user.user_type != 'ADMIN' and request.user.id != user.id:
            return Response(
...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated and hasattr(user, 'user_type') and user.user_type == 'STUDENT':
            ...
            logger.info(f"User {user.username} is NOT a STUDENT or not authenticated. Showing all non-deleted courses.")
    ...
    def enroll(self, request, pk=None):
        ...
        if hasattr(user, 'user_type') and user.user_type == 'STUDENT':
            ...
            # Count only truly active enrollments and normalize stale statuses first.
            # This ensures that once one of two enrolled courses is completed,
            # the student can enroll in one new course immediately.
...
def live_notifications(request):
    """
    Lightweight endpoint for polling new notifications.
    Clients can pass ?since=<ISO timestamp> to fetch only newer rows.
    """
...
def resolve_request(request, request_id):
    ...
    elif delete_request.request_type == 'STUDENT_REGISTRATION':
        User.objects.filter(
            id=delete_request.object_id,
            user_type='STUDENT',
            is_deleted=False
...
def undo_request(request, request_id):
    ...
    try:
        if delete_request.request_type == 'DELETE_COURSE':
            course = Course.objects.get(id=delete_request.object_id)
            course.is_deleted = False
            course.save()
        elif delete_request.request_type == 'DELETE_VIDEO':
            video = Video.objects.get(id=delete_request.object_id)
            video.is_deleted = False
            video.save()
        elif delete_request.request_type == 'DELETE_QUIZ':
            quiz = Quiz.objects.get(id=delete_request.object_id)
            quiz.is_deleted = False
            quiz.save()
        elif delete_request.request_type == 'DELETE_CATEGORY':
            category = Category.objects.get(id=delete_request.object_id)
            category.is_deleted = False
            category.save()
        elif delete_request.request_type == 'DELETE_TEAM':
            team = Team.objects.get(id=delete_request.object_id)
            team.is_deleted = False
            team.save()
        elif delete_request.request_type == 'DELETE_USER':
            user = User.objects.get(id=delete_request.object_id)
...
def permanent_delete_request(request, request_id):
    ...
    if delete_request.status != 'APPROVED':
        return Response(
            {"error": "Only approved requests can be permanently deleted"},
            status=status.HTTP_400_BAD_REQUEST
        )
